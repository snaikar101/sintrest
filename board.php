<?php 
		function createBoard($sql_con){
			$name_board=get_input_post('Name');
                        ...
                        $Catogery= get_input_post('Catogery');
			...
			if(isset($_POST['only_friends']))
			{
				$only_friends= 'TRUE';
                        ...
                        $query = "insert into board (name,description,category,only_friends,created_by,created_date) values('$name_board','$Description','$Catogery','$only_friends','$current_user',now());";
                        $result=mysqli_query($sql_con,$query);
                        if(!$result){
				echo "<p class='text-danger' style='padding-top:5%'>failed creating board.Board name must be unique</p>";
			}
			else{
				echo "<p class='text-success' style='padding-top:5%'>$name_board board created</p>";
			}
			...
			while($row = mysqli_fetch_array($result)){
				...
				$result1=mysqli_query($sql_con,$query1);
						...
						echo"<div class='col-xs-6'>";
                                                echo"<img class='img-thumbnail' src='$path' style='height:50px;width: 100%;'/>";     
						echo"</div>";
						}   
					}
				}
