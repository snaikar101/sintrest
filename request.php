...
<?php
		...
		if(mysqli_num_rows($result1)==1)
		{
			...
			mysqli_query($sql_con,$query3);
                	echo "<p class='text-success' style='padding-top:5%'>request successfully accepted</p>";}
			elseif($_GET['i']==-1){
                        $query ="delete from friend where user_id='$user_id' and friend_id = '$current_user';";
                        //echo $query;
                        //exit;
			mysqli_query($sql_con,$query);
                        echo "<p class='text-success' style='padding-top:5%'>request successfully rejected</p>";}
		}
        }
?>
</head>
...
<?php
                        ...
                        if(mysqli_num_rows($result) == 0){
                                echo "<p class='text-warning'>No Freind requests</p>";
                        }
